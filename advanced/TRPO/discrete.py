...
# GAE,给定gamma,lmbda,每个时间步的δ-t,进行优势估计
...
class TRPO:
    ...
    def hessian_matrix_vector_product(self, states, old_action_dists, vector):
        ...
        kl = torch.mean(
            torch.distributions.kl.kl_divergence(old_action_dists,
                                                 new_action_dists))
        kl_grad = torch.autograd.grad(kl, self.actor.parameters(),
                                      create_graph=True)
        # 大饼切开拼成面条
    ...
    def compute_surrogate_obj(self, states, actions, advantage,
                              old_log_probs, actor):
        # 计算策略目标
    ...
    def line_search(self, states, actions, advantage,
        ...
        for i in range(15):
            ...
            # new_actor就是从self.actor拷贝来的，在这算新目标
            # 新目标效果更好那就更新参数，否则用旧的
    ...
    def policy_learn(self, states, actions,
                     old_action_dists, old_log_probs, advantage):
        # 更新策略函数
        ...
        # 反正是把max_coef搞出来了，用这个来线性搜索
        new_para = self.line_search(states, actions, advantage,
    ...
    def update(self, transition_dict):
        ...
        self.critic_optimizer.step() # 更新价值/老师函数
...
if __name__ == '__main__':
    ...
    lmbda = 0.95
    critic_lr = 1e-2
    # kl_constraint = 0.0005​ - 新旧策略间的KL散度上限。
    # 这是TRPO的"安全带"：​每次更新策略时，新策略不能偏离旧策略太远。
    # 0.0005是很小的约束，确保更新稳定。
    kl_constraint = 0.0005
    # alpha = 0.5​ - 回溯系数。
    # 当一次更新违反KL约束时，用这个系数缩小步长重新尝试。
    # 0.5意味着"先迈一大步，如果出界就退回一半"
    alpha = 0.5
    ...
    plt.xlabel('Episodes')
    plt.ylabel('Returns')
    plt.title('TRPO ON {}\nepisode_return'.format(env_name))
    plt.show()
